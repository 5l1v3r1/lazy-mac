...
εᴳ-simᴸ⋆ {ls = ls} (suc n₂) Σ₁≈Σ₂ L⊑A step ⟨ Σ₁≈Σ₃ , P₁≈P₂ , Γ₁≈Γ₂ ⟩
  | Σ₂' P., H P., m P., H⋤A P., Σ₂≈Σ₂' P., nextˢ | T₂ P., T∈P₂
  | C.⟨ fork .H⊑H₂ .tᴴ , S₂ ⟩ P., t∈T₂ | isR (Step {p' = ⟨ Γ₂' , t₂' , S₂' ⟩} step₂)
  | yes (Fork {h = H₂} H⊑H₂ tᴴ) with updateᵀ t∈T₂ ⟨ t₂' , S₂' ⟩
... | T₂' P., uᵀ with updateᴾ T∈P₂ T₂'
... | P₂' P., uᴾ with ⟨ forget Σ₂≈Σ₂' , trans-≈ᴾ P₁≈P₂ L₁.⌜ updateᴾ∙ H⋤A uᴾ ⌝ᴾ , trans-≈ᴴ Γ₁≈Γ₂ ⌜ stepᴴ-Γ H⋤A step₂ ⌝ᴴ ⟩
... | g₂≈g₂' with lookupᴾ (H₂ ∈ᴸ ls) P₂'
... | T₂ᴴ P., H∈P₂' with εᴳ-simᴸ⋆ n₂ Σ₂≈Σ₂' L⊑A step g₂≈g₂'
... | Cᴳ g₂'' g₂'≈g₂'' ss
-- No clue why the labels are  wrong
  = Cᴳ _ g₂'≈g₂'' (fork {l = H} {H = H₂} {tᴴ = tᴴ} {l⊑H = {!H⊑H₂!}} T∈P₂ {!t∈T₂!} {!step₂!} uᵀ uᴾ H∈P₂' (nextˢ (Fork H₂ (lengthᵀ T₂ᴴ) {!!})) {!!} ∷ ss)

εᴳ-simᴸ⋆ (suc n₂) Σ₁≈Σ₂ L⊑A step ⟨ Σ₁≈Σ₃ , P₁≈P₂ , Γ₁≈Γ₂ ⟩
  | Σ₂' P., H P., m P., H⋤A P., Σ₂≈Σ₂' P., nextˢ | T₂ P., T∈P₂
  | C.⟨ ._ , S₂ ⟩ P., t∈T₂ | isR (Step {p' = ⟨ Γ₂' , t₂' , S₂' ⟩} step₂) | yes (Fork∙ l⊑H t₂ᴴ) with updateᵀ t∈T₂ ⟨ t₂' , S₂' ⟩
... | T₂' P., uᵀ with updateᴾ T∈P₂ T₂'
... | P₂' P., uᴾ with ⟨ forget Σ₂≈Σ₂' , trans-≈ᴾ P₁≈P₂ L₁.⌜ updateᴾ∙ H⋤A uᴾ ⌝ᴾ , trans-≈ᴴ Γ₁≈Γ₂ ⌜ stepᴴ-Γ H⋤A step₂ ⌝ᴴ ⟩
... | g₂≈g₂' with εᴳ-simᴸ⋆ n₂ Σ₂≈Σ₂' L⊑A step g₂≈g₂'
-- Same weird error
