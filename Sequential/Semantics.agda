...
data _⇝_ {ls : List Label} {l : Label} : ∀ {τ} -> State ls l τ -> State ls l τ -> Set where
 ...
 App₁ : ∀ {τ₁ τ₂ τ₃ Γ Γ' π} {Δ Δ' : Env l π} {t₁ : Term π (τ₁ => τ₂)} {t₂ : Term π τ₁} {S : Stack l τ₂ τ₃} ->
          let x = ⟪ length π , τ₁ , l ⟫ in
             (Δ∈Γ : l ↦ Δ ∈ᴴ Γ)  -- Do we need this?
          -> (uᴱ : Δ' ≔ Δ [ x ↦ t₂ ]ᴱ)
          -> (uᴴ : Γ' ≔ Γ [ l ↦ Δ' ]ᴴ) ->
          ⟨ Γ , (App t₁ t₂) , S ⟩ ⇝ ⟨ Γ' , t₁ , (Var {π = x ∷ π} here) ∷ S ⟩

 App₂ : ∀ {Γ n₁ n₂ β l' α τ'} {π : Context} {S : Stack l β τ'} ->
          let x = ⟪ n₁ , α , l' ⟫
 ...
 Var₁ : ∀ {Γ Γ' n τ τ'} {π : Context} {Δ Δ' : Env l π}  {S : Stack l τ τ'} {t : Term π τ} ->
          let x = ⟪ n , τ , l ⟫ in
           (Δ∈Γ : l ↦ Δ ∈ᴴ Γ)
        -> (x∈π : x ∈ π)
        -> (t∈Δ : x ↦ t ∈ᴱ Δ)
 ...
 Var₁' : ∀ {Γ l' τ n τ'} {π : Context} {Δ : Env l' π} {S : Stack l τ τ'} ->
 ...
 Var₂ : ∀ {Γ Γ' n τ τ'} {π : Context} {Δ Δ' : Env l π} {S : Stack l τ τ'} {v : Term π τ} ->
          let  x = ⟪ n , τ , l ⟫ in
           (Δ∈Γ : l ↦ Δ ∈ᴴ Γ)
        -> (x∈π : x ∈ π)
        -> (val : Value v)
